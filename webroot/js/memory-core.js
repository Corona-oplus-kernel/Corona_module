...
,
    ...
    initializeHomeInteractions() {
        ...
        const loadExtras = () => this.loadHomeExtras().catch(error => console.error('home extras failed', error));
    ...
    async loadHomeExtras() {
        if (this.homeExtrasPromise) return this.homeExtrasPromise;
        this.homeExtrasPromise = this.ensureFeatureScript('memory-extras').then(() => {
            this.initBannerDrag();
            this.initEasterEgg();
        }).catch(error => {
            this.homeExtrasPromise = null;
            throw error;
        });
        return this.homeExtrasPromise;
    },
