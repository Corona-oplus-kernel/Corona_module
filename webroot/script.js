class CoronaAddon {
    ...
    setLanguage(language, persist = false) {
        ...
        this.updateLanguagePickerState();
    }
    applyTranslations() {
    ...
    updateLanguagePickerState() {
        ...
        if (!picker || !trigger || !value) return;
        value.textContent = this.state.language === 'en' ? 'English' : '中文';
        picker.querySelectorAll('.language-picker-option').forEach(option => {
            option.classList.toggle('active', option.dataset.value === this.state.language);
        });
    }
    closeLanguagePicker() {
        const picker = document.getElementById('language-picker');
    ...
    initLanguageToggle() {
        const picker = document.getElementById('language-picker');
        const trigger = document.getElementById('language-select');
        if (!picker || !trigger || trigger.dataset.bound) return;
        ...
        picker.querySelectorAll('.language-picker-option').forEach(option => {
            option.addEventListener('click', () => {
                const next = option.dataset.value || 'zh';
                this.setLanguage(next, true);
                this.closeLanguagePicker();
                this.showToast(`${this.t('languageLabel')}: ${option.textContent || next}`);
            });
        });
        document.addEventListener('click', (e) => {
            if (!picker.contains(e.target)) this.closeLanguagePicker();
        });
    }
    ...
    initResetAllBtn() {
        ...
        const toggle = document.getElementById('reset-config-toggle');
        if (!btn || btn.dataset.bound) return;
        btn.dataset.bound = '1';
        btn.addEventListener('click', () => this.resetAllSettings());
        if (toggle && !toggle.dataset.bound) {
            toggle.dataset.bound = '1';
            toggle.addEventListener('click', () => this.toggleResetConfigPanel());
        }
        this.updateResetAllButtonLabel(false);
    }
    updateResetAllButtonLabel(expanded) {
        const text = document.querySelector('#reset-all-btn .reset-all-btn-text');
        if (text) text.textContent = expanded ? '重置全部配置并重启' : '一键重置';
    }
    getResettableConfigEntries() {
        return [
            ['zram', 'ZRAM 配置', 'zram.conf'],
            ['swap', 'Swap 配置', 'swap.conf'],
            ['io', 'I/O 配置', 'io_scheduler.conf'],
            ['vm', 'VM 配置', 'vm.conf'],
            ['kernel', 'Kernel 配置', 'kernel.conf'],
            ['le9ec', 'LE9EC 配置', 'le9ec.conf'],
            ['cpu_governor', 'CPU 调速器', 'cpu_governor.conf'],
            ['cpu_hotplug', 'CPU 热插拔', 'cpu_hotplug.conf'],
            ['tcp', 'TCP 配置', 'tcp.conf'],
            ['lmk', 'LMK 配置', 'lmk.conf'],
            ['reclaim', 'Reclaim 配置', 'reclaim.conf'],
            ['kswapd', 'Kswapd 配置', 'kswapd.conf'],
            ['protect', '应用保护', 'protect.conf'],
            ['fstrim', 'Fstrim 配置', 'fstrim.conf'],
            ['device', '设备参数', 'device.conf'],
            ['corona_kernel', 'Corona Kernel', 'corona_kernel.conf'],
            ['process_priority', '进程优先级', 'process_priority.conf'],
            ['thread_priority', '线程规则', 'thread_priority.conf'],
            ['app_rules', '应用策略', 'app_rules.conf'],
            ['app_whitelist', '应用白名单', 'app_whitelist.list'],
            ['app_protect', '保护进程名单', 'app_protect.list'],
            ['app_profiles_list', '应用预设名单', 'app_profiles.list'],
            ['app_profiles', '应用预设目录', 'app_profiles', 'dir'],
            ['parameter_snapshots', '参数快照', 'parameter_snapshots.b64'],
            ['custom_scripts', '自定义脚本', 'custom_scripts.b64'],
            ['runtime', '运行时配置', 'runtime.conf'],
            ['dual_cell', '双电芯配置', 'dual_cell.conf'],
            ['app_meta_cache', '应用元数据缓存', 'app_meta_cache.b64']
        ].map(([key, label, path, type]) => ({ key, label, path, type: type || 'file' }));
    }
    async renderResettableConfigs() {
        const list = document.getElementById('reset-config-list');
        if (!list) return;
        const entries = this.getResettableConfigEntries();
        const checks = await Promise.all(entries.map(async (item) => {
            const fullPath = `${this.configDir}/${item.path}`;
            const testCmd = item.type === 'dir'
                ? `[ -d ${this.shellQuote(fullPath)} ] && echo 1 || echo 0`
                : `[ -f ${this.shellQuote(fullPath)} ] && echo 1 || echo 0`;
            const exists = (await this.exec(testCmd)).trim() === '1';
            return exists ? item : null;
        }));
        const available = checks.filter(Boolean);
        if (available.length === 0) {
            list.innerHTML = '<div class="danger-config-empty">当前没有可单独删除的配置</div>';
            return;
        }
        list.innerHTML = available.map(item => `
            <div class="danger-config-item">
                <div class="danger-config-info">
                    <div class="danger-config-name">${this.escapeHtml(item.label)}</div>
                    <div class="danger-config-path">${this.escapeHtml(item.path)}</div>
                </div>
                <button class="danger-config-delete" data-key="${this.escapeHtml(item.key)}">删除</button>
            </div>
        `).join('');
        list.querySelectorAll('.danger-config-delete').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                const key = e.currentTarget.dataset.key || '';
                await this.deleteSingleConfig(key);
            });
        });
    }
    async toggleResetConfigPanel(force = null) {
        const toggle = document.getElementById('reset-config-toggle');
        const panel = document.getElementById('reset-config-panel');
        if (!toggle || !panel) return;
        const next = typeof force === 'boolean' ? force : !panel.classList.contains('expanded');
        toggle.classList.toggle('expanded', next);
        panel.classList.toggle('expanded', next);
        toggle.setAttribute('aria-expanded', next ? 'true' : 'false');
        this.updateResetAllButtonLabel(next);
        if (next) await this.renderResettableConfigs();
    }
    async deleteSingleConfig(key) {
        const entry = this.getResettableConfigEntries().find(item => item.key === key);
        if (!entry) return;
        const confirmed = await this.showConfirm(`确定要删除 ${entry.label} 吗？\n\n仅删除 ${entry.path}，不会重启设备。`, '删除指定配置');
        if (!confirmed) return;
        const fullPath = `${this.configDir}/${entry.path}`;
        await this.exec(entry.type === 'dir' ? `rm -rf ${this.shellQuote(fullPath)} 2>/dev/null` : `rm -f ${this.shellQuote(fullPath)} 2>/dev/null`);
        await this.renderResettableConfigs();
        this.showToast(`${entry.label} 已删除`);
    }
    ...
    initThreadRuleUi() {
        ...
        document.getElementById('thread-rule-cancel-btn')?.addEventListener('click', () => { this.hideOverlay('thread-rule-editor-overlay'); this.showOverlay('thread-rule-overlay'); this.setActiveThreadPanel('rules'); });
        document.getElementById('thread-rule-save-btn')?.addEventListener('click', () => this.saveThreadRule());
        document.querySelectorAll('[data-panel-toggle]').forEach(btn => btn.addEventListener('click', () => this.setActiveThreadPanel(btn.dataset.panelToggle || 'rules')));
        ...
        document.querySelectorAll('.thread-io-option').forEach(opt => { opt.addEventListener('click', () => { document.querySelectorAll('.thread-io-option').forEach(o => o.classList.remove('selected')); opt.classList.add('selected'); opt.animate([{ transform: 'scale(0.98)' }, { transform: 'scale(1.02)' }, { transform: 'scale(1)' }], { duration: 220, easing: 'ease-out' }); this.selectedThreadIoClass = parseInt(opt.dataset.class, 10); this.selectedThreadIoLevel = parseInt(opt.dataset.level, 10); }); });
    }
    ...
    renderThreadChips(containerId, items, onPick, options = {}) {
        const container = document.getElementById(containerId);
        if (!container) return;
        const normalized = (items || []).map(item => typeof item === 'string' ? { pattern: item, label: item, desc: '' } : item).filter(item => item && item.pattern);
        const unique = normalized.filter((item, index, list) => list.findIndex(other => other.pattern === item.pattern) === index);
        if (unique.length === 0) { container.innerHTML = `<div class="priority-empty">${this.escapeHtml(options.emptyText || '暂无可用线程')}</div>`; return; }
        container.innerHTML = unique.map(item => `<div class="thread-chip" data-value="${this.escapeHtml(item.pattern)}"><div class="thread-chip-name">${this.escapeHtml(item.label || item.pattern)}</div><div class="thread-chip-pattern">${this.escapeHtml(item.desc || item.pattern)}</div></div>`).join('');
    ...
    async ensureLiveThreadSuggestions(force = false) {
        ...
        if (!pkg || this.threadSuggestionLoading) return;
        const container = document.getElementById('thread-rule-live-list');
        if (!force && this.threadSuggestionCache && Array.isArray(this.threadSuggestionCache[pkg])) {
            this.renderThreadChips('thread-rule-live-list', this.threadSuggestionCache[pkg].map(item => ({ pattern: item, label: item, desc: '当前进程线程' })), (pattern) => this.openThreadRuleEditor(null, pattern), { emptyText: '暂无运行线程' });
            return;
        }
        this.threadSuggestionLoading = true;
        if (container) container.innerHTML = '<div class="priority-loading">正在读取...</div>';
        try {
            const liveThreads = await this.loadLiveThreadSuggestions(pkg, force);
            if (this.selectedThreadRulePackage !== pkg) return;
            this.renderThreadChips('thread-rule-live-list', liveThreads.map(item => ({ pattern: item, label: item, desc: '当前进程线程' })), (pattern) => this.openThreadRuleEditor(null, pattern), { emptyText: '暂无运行线程' });
        } catch (error) {
            if (this.selectedThreadRulePackage !== pkg) return;
            if (container) container.innerHTML = '<div class="priority-empty">读取线程失败</div>';
        } finally {
            this.threadSuggestionLoading = false;
        }
    }
    renderThreadRuleList() {
        ...
        if (rules.length === 0) {
            list.innerHTML = '<div class="priority-empty">该应用还没有线程规则，点下方常用线程或运行线程即可添加</div>';
            return;
        }
    ...
    async openThreadRuleManager(pkg, label) {
        ...
        this.renderThreadChips('thread-rule-common-list', this.getCommonThreadPresets(), (pattern) => this.openThreadRuleEditor(null, pattern), { emptyText: '暂无常用模式' });
        const live = document.getElementById('thread-rule-live-list');
        if (live) live.innerHTML = '<div class="priority-empty">展开后读取</div>';
        this.hideOverlay('app-policy-overlay');
        this.showOverlay('thread-rule-overlay');
        this.setActiveThreadPanel(this.getThreadRulesForPackage(pkg).length > 0 ? 'rules' : 'common');
    }
