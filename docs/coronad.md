...
守护进程启动后会主动迁移到 system cgroup，避免从 KernelSU WebUI 启动时继承应用冻结组并在 WebUI 退到后台后停止响应。

动态调节复用守护进程已有采样周期，不启动额外线程或轮询。只有档位实际变化时才探测并写入可用节点；状态文件仅记录与基线不同的参数、差值和目标值，恢复基线后自动清空动态参数列表。
