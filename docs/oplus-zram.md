...
OPlus ERM 的后端识别、自适应回收和官方基线恢复见 [OPlus ERM 自适应策略](oplus-erm.md)。

## 目标

Corona 不替换 OPlus 的完整启动链。模块等待官方 ZRAM 初始化完成，再对 `config/zram.conf` 中存在的字段做增量覆盖；未写入的字段保持官方值。

自适应策略启用后由 `coronad` 调用 `scripts/zram-policy.sh tick`，不再启动独立的 `zram-policy.sh daemon`。屏幕、温度和内存 PSI 由 `coronad` 传入，脚本只读取 ZRAM、HybridSwap 和 ERM 专用节点并返回下一次执行间隔。
